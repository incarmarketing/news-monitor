  ...
  const response = await fetch(
    `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${apiKey}`,
    {
      ...
      body: JSON.stringify({
        systemInstruction: {
          parts: [{
            text: "당신은 보험/GA 업계 언론홍보 리스크 대응 초안을 작성하는 한국어 PR 실무자입니다. 기사 주장을 그대로 반복하지 말고, 쟁점·사실확인·대응원칙·문의응대 문구로 재구성합니다. 사실 확인 전 단정 표현을 피하고, 법적 책임 인정처럼 보이는 표현을 쓰지 않습니다.",
          }],
        },
