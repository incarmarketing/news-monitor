...
class SlackDailyPayloadTests(unittest.TestCase):
    def test_daily_payload_uses_compact_metrics_and_headline_only(self) -> None:
        ...
        self.assertEqual(len(metric_block["column_settings"]), 5)
        self.assertEqual(len(metric_block["rows"][0]), 5)
        self.assertEqual(metric_block["rows"][1][0]["text"], "LOW")
        self.assertEqual(metric_block["rows"][0][1]["text"], "\ubd84\uc11d")
        self.assertEqual(metric_block["rows"][1][1]["text"], "2")
        self.assertEqual(metric_block["rows"][0][2]["text"], "\uae0d\uc815")
        self.assertEqual(metric_block["rows"][0][3]["text"], "\uc911\ub9bd")
        self.assertEqual(metric_block["rows"][0][4]["text"], "\ubd80\uc815")
        self.assertEqual(metric_block["rows"][1][2]["text"], "1")
        self.assertEqual(metric_block["rows"][1][3]["text"], "0")
        self.assertEqual(metric_block["rows"][1][4]["text"], "0")
