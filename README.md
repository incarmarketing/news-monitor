...
GitHub Actions 자동 실행은 한국시간 08:00, 13:00, 18:00 기준으로 동작합니다. 예약 지연에 대비해 각 기준시각의 05분, 15분에 백업 실행을 걸고, 성공한 기준시각은 `.run-state/` 마커로 중복 발송을 막습니다.

수집 구간은 업무 흐름 기준입니다.
- 08시 보고서: 전일 18:00부터 현재까지
- 13시 보고서: 당일 08:00부터 현재까지
- 18시 보고서: 당일 13:00부터 현재까지
...
### 실행 시간 변경 → `config.py`
```python
SCHEDULE_TIMES = ["08:00", "13:00", "18:00"]   # 24시간 형식
```
