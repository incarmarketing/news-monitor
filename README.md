...
`minutes_back=5`는 “한 번 실행할 때 최근 5분을 본다”는 뜻이고, 실제 실행 주기도 5분이어야 합니다. 대시보드에 `실제 호출 약 10분`처럼 보이면 외부 cron이 아직 10분 단위로 남아 있는 상태입니다. 이때는 GitHub Actions의 `Sync External Cron`을 실행해 cron-job.org 설정을 다시 동기화합니다.
...
- `Negative Article Watch`: 24시간 5분 단위 부정기사 감시
...
1. GitHub Actions > `Negative Article Watch` 실행 간격을 확인합니다.
...
4. 대시보드의 `실제 호출 약 n분` 표기가 5분대로 내려오는지 확인합니다.
