...
- 실행 방식: 30분마다 클라우드 러너를 띄우고, 러너 안에서 5분 간격으로 6회 실제 감시
- 보조 호출: cron-job.org `news-monitor negative watch`
- 저장 위치: Supabase `negative_watch_runs`, `.watch-state`

`minutes_back=5`는 “한 번 실행할 때 최근 5분을 본다”는 뜻이고, 실제 감시도 5분마다 수행되어야 합니다. 대시보드에 `실제 약 10분`처럼 보이면 최근 실행 기록이 아직 5분 간격으로 쌓이지 않았거나 외부 cron이 예전 설정으로 남아 있는 상태입니다. 이때는 `Negative Article Watch` 실행 기록과 `Sync External Cron`을 함께 확인합니다.
...
- `Negative Article Watch`: 24시간 5분 단위 부정기사 감시. 예약은 30분 단위지만, 실행된 클라우드 러너 안에서 5분 간격으로 6회 감시합니다.
